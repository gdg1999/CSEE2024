...
def picture11(combined_data):
    ...
    # 绘制园区C的数据
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C光伏出力(kW)'], width=bar_width, label='Park C PV Output (kW)', color='lightpink', alpha=alpha)
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C风电出力(kW)'], width=bar_width, bottom= combined_data['园区C光伏出力(kW)'], label='Park C Wind Output (kW)', color='skyblue', alpha=alpha)
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C负荷(kW)'], width=bar_width, label='Park C Load (kW)', edgecolor='black', facecolor='none', linewidth=2, linestyle = '--')
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C购电量(kW)'], width=bar_width, bottom= combined_data['园区C光伏出力(kW)']+combined_data['园区C风电出力(kW)'], label='Park C Purchase (kW)', color='orange', alpha=alpha)
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C弃电量(kW)'], width=bar_width, bottom = -combined_data['园区C弃电量(kW)'], label='Park C Excess (kW)', color='gray', alpha=alpha)
...
def picture12(combined_data):
    ...
    # 绘制园区A的数据
    axes[0].bar(combined_data['时间（h）'], combined_data['园区A光伏出力(kW)'], width=bar_width, label='Park A PV Output (kW)', color='lightpink', alpha=alpha)
    bars_charge = axes[0].bar(combined_data['时间（h）'], combined_data['园区A_充放电(kWh)'], width=bar_width, bottom= combined_data['园区A负荷(kW)'], label='Park A Charge (kW)', color='purple', alpha=1)
    axes[0].bar(combined_data['时间（h）'], combined_data['园区A负荷(kW)'], width=bar_width, label='Park A Load (kW)', edgecolor='black', facecolor='none', linewidth=2, linestyle = '--')
    ...
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C光伏出力(kW)'], width=bar_width, label='Park C PV Output (kW)', color='lightpink', alpha=alpha)
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C风电出力(kW)'], width=bar_width, bottom= combined_data['园区C光伏出力(kW)'], label='Park C Wind Output (kW)', color='skyblue', alpha=alpha)
    bars_charge = axes[2].bar(combined_data['时间（h）'], combined_data['园区C_充放电(kWh)'], width=bar_width, bottom= combined_data['园区C负荷(kW)'], label='Park C PV Charge (kW)', color='purple', alpha=1)
    axes[2].bar(combined_data['时间（h）'], combined_data['园区C负荷(kW)'], width=bar_width, label='Park C Load (kW)', edgecolor='black', facecolor='none', linewidth=2, linestyle = '--')
